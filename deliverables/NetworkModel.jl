...
function initialize_network(network::ThreeBusNetwork, V_m::Vector{Float64}, θ::Vector{Float64}, P::Vector{Float64}, Q::Vector{Float64})
    ...
    b = [v_d[1] - v_d[2];
        ...
        v_q[2] - v_q[3]]
    line_currents = A \ b
    println("Line Currents: $line_currents")
    ...
    # Sanity check on line currents (this is just a KCL on the calculated initial values)
    res_i = [
...
function update_network_states!(
    ...
    # Calculate S_load for sanity check
    P_load = v_3_d * i_3_d + v_3_q * i_3_q
    Q_load = v_3_q * i_3_d - v_3_d * i_3_q
    S_load = complex(P_load, Q_load)
    V_load_mag = abs(complex(v_3_d, v_3_q))
    Z_load_test = (V_load_mag^2) / conj(S_load)
    #sanity_check(Z_load_test, network.Z_L, "Load in integrator", false)
    ...
    return V_terminal, S_terminal, I_terminal, i_2_d, i_2_q, Z_load_test
end
end # module