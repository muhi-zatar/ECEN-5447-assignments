...
export SauerPaiMachine, initialize_machine_states, update_machine_states!
export DELTA, OMEGA, EQ_P, ED_P, PSI_D_PP, PSI_Q_PP, PSI_D, PSI_Q
...
# State indices for machine
const DELTA = 1     # Rotor angle
const OMEGA = 2     # Angular frequency
const EQ_P = 3      # q-axis transient voltage
const ED_P = 4      # d-axis transient voltage
const PSI_D_PP = 5  # d-axis subtransient flux
const PSI_Q_PP = 6  # q-axis subtransient flux
const PSI_D = 7     # d-axis stator flux
const PSI_Q = 8     # q-axis stator flux
...
Synchronous machine model using Sauer-Pai formulation.
"""
mutable struct SauerPaiMachine
    # Machine parameters
    R::Float64           # Armature resistance
    X_d::Float64         # d-axis synchronous reactance
    X_q::Float64         # q-axis synchronous reactance
    ...
    Td0_pp::Float64      # d-axis subtransient open-circuit time constant
    Tq0_pp::Float64      # q-axis subtransient open-circuit time constant
    γ_d1::Float64        # d-axis model coefficient 1
    γ_q1::Float64        # q-axis model coefficient 1
    γ_d2::Float64        # d-axis model coefficient 2
    γ_q2::Float64        # q-axis model coefficient 2
    H::Float64           # Inertia constant
    D::Float64           # Damping coefficient
    base_power::Float64                  # Machine base power
    system_base_power::Float64           # System base power
    system_base_frequency::Float64       # System base frequency (Hz)
    M::AbstractArray{Float64}            # Mass matrix components for ODE solver
    ...
    function SauerPaiMachine(;
        ...
        return new(R, X_d, X_q, Xd_p, Xq_p, Xd_pp, Xq_pp, Xl,
            ...
            γ_d1, γ_q1, γ_d2, γ_q2, 
            H, D, base_power,
            system_base_power, system_base_frequency, M)
    end
end

# Accessor functions for machine parameters
...
"""
    initialize_machine_states(machine::SauerPaiMachine, V_terminal, delta, P, Q)

Initialize machine states based on power flow solution.
Returns the initial states, field voltage, and mechanical torque.
"""
function initialize_machine_states(machine::SauerPaiMachine, V_terminal, delta, P, Q)
    # Extract machine parameters for easier access
    R = get_R(machine)
    Xd = get_Xd(machine)
    Xq = get_Xq(machine)
    ...
    @info "Initial S_bus = $(complex(p_bus, q_bus))"

    # Initialize states for steady state
    ...
    # Solve using NLsolve for more accurate steady-state solution
    function f!(out, x)
        δ = x[1]
        τm = x[2]
        Vf0 = x[3]
    ...
    if !NLsolve.converged(sol)
        @warn "Initialization in machine failed! Using approximate solution."
        states[DELTA] = δ
        states[OMEGA] = 1.0
        states[EQ_P] = eq_p
        states[ED_P] = ed_p
...
"""
    update_machine_states!(states, derivatives, V_terminal, Vf, τ_m, machine)

Update machine states based on current values.
Returns current injection, power, speed, and terminal voltage and current magnitudes.
"""
function update_machine_states!(
    ...
    # Transform voltage to machine reference frame
    V_dq = ri_dq(δ) * [real(V_terminal); imag(V_terminal)]
    v_d = V_dq[1]       # Eqn 15.4
    v_q = V_dq[2]       # Eqn 15.4
    V_mag = abs(complex(v_d, v_q))  # Voltage magnitude for monitoring

    # Calculate currents from flux linkages
    ...
    τ_e = ψ_d * i_q - ψ_q * i_d
    I_mag = abs(complex(i_d, i_q))  # Current magnitude for monitoring
    ...
    # Transient voltage equations (15.13 in Milano's book)
    ...
    I_RI = dq_ri(δ) * [i_d; i_q]
    I_terminal = complex(I_RI[1], I_RI[2])

    return I_terminal, S_bus, ω, V_mag, I_mag
end
